                      ...
                      arrow_btsp_result *tour_result)
{
    ...
    solve_problem = btsp_problem;
    solve_problem2 = mstsp_problem;
    ...
    *lb_time = 0.0;
    ...
    arrow_debug("Starting double bottleneck search\n");
        ...
        else
        {
            ...
            if(cur_tour_result.found_tour)
            {
                ...
                if(cur_gap < best_gap)
                {
                    ...
                    {
                        for(k = 0; k < solve_problem->size; k++)
                ...
                /* Solve an MSTSP to see if we can push the lower bound up */
                arrow_debug("Starting MSTSP search to raise lower index\n");
            
                /* Create problem that ignores C[i,j] >= low_val */
                if(!arrow_btsp_fun_apply(&fun_ib, solve_problem2, 
                                         info->max_cost - info->cost_list[high], 
                                         info->max_cost, &ib_problem))
                {
                    arrow_print_error("Could not create IB cost matrix\n");
                    ret = ARROW_FAILURE;
                    goto CLEANUP;
                ...
                if(!arrow_btsp_solve(&ib_problem, info, btsp_params, &cur_tour_result))
                {
                    arrow_print_error("Error searching for MSTSP tour\n");
                    ret = ARROW_FAILURE;
                    goto CLEANUP;
                }
                end_time = arrow_util_zeit();
                ...
                if(is_feasible)
                {
                    ...
                    /* Conver the MSTSP costs into BTSP costs */
                    min_cost = info->max_cost - cur_tour_result.max_cost;
                    max_cost = info->max_cost - cur_tour_result.min_cost;
                    
                    arrow_debug("  - Tour MinCost = %d, MaxCost = %d\n", min_cost, max_cost);
        
                    best_gap = best_tour_high - best_tour_low;
                    cur_gap = max_cost - min_cost; 
        
                    /* Find the indices for the min cost in tour */
                    if(!arrow_problem_info_cost_index(info, min_cost, &p2))
                    {
                        arrow_print_error("Could not find min_cost in cost_list\n");
                        ret = ARROW_FAILURE;
                        goto CLEANUP;
                    ...
                    if(cur_gap < best_gap)
                    {
                        ...
                        tour_result->min_cost = min_cost;
                        tour_result->max_cost = max_cost;
                        tour_result->tour_length = 
                            info->max_cost * solve_problem->size - cur_tour_result.tour_length;
        
                        if(tour_result->tour != NULL)
                        {
                            tour_result->found_tour = ARROW_TRUE;
                            if(solve_problem->symmetric)
                            {
                                for(i = 0; i < solve_problem->size; i++)
                            ...
                            else
                            {
                                arrow_util_sbtsp_to_abstp_tour(solve_problem, cur_tour_result.tour, 
                                                               tour_result->tour);
                                tour_result->tour_length += solve_problem->size * btsp_params->infinity;
                            }
                        }
    
                        best_tour_low = min_cost;
                        best_tour_high = max_cost;
