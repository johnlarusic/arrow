...
arrow_option options[NUM_OPTS] = 
{
    ...
    {'a', "dt2-attempts", "number of dt2 attempts",
        ARROW_OPTION_INT, &dt2_attempts, ARROW_FALSE, ARROW_TRUE},
    {'b', "basic-attempts", "number of basic attempts",
...
{   
    ...
    if(!arrow_baltsp_fun_ut(deep_copy, &fun_basic))
        return EXIT_FAILURE;
    if(!arrow_baltsp_fun_dt2(deep_copy, shake_rand_min, shake_rand_max, &info, &fun_dt2))
        return EXIT_FAILURE;
    ...
    arrow_btsp_solve_plan steps[SOLVE_STEPS] = 
    {
       ...
       },
       {
           ARROW_TSP_CC_LK,                 /* TSP solver */
           (void *)&lk_params,              /* TSP solver parameters */
           fun_dt2,                         /* fun (cost matrix function) */
           dt2_attempts                     /* attempts */
       }
    };
    ...
    baltsp_params.num_steps         = SOLVE_STEPS;
    baltsp_params.steps             = steps;
    baltsp_params.infinity          = edge_infinity;
    baltsp_params.deep_copy         = deep_copy;

    
    /* Setup BalTSP results structure and solve BTSP */
    arrow_btsp_result_init(&problem, &tour_result);
    if(!arrow_balanced_tsp_dt2(&problem, &info, &baltsp_params, &lb_time, &tour_result))
    {
