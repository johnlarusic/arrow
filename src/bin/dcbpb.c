/**********************************************************doxygen*//** @file
 * @brief   Degree Constrained Bottleneck Path Bound solver.
 *
 * Solves the Degree Constrained Bottleneck Path Bound (DCBPB) on the given 
 * input file.
 *
 * @author  John LaRusic
 * @ingroup bin
...
{   
    ...
    /* Solve BAP */
    if(!arrow_dcbpb_solve(&problem, &result))
    {
        arrow_print_error("Could not solve DCBPB on file.");
        return EXIT_FAILURE;
    }
    ...
    if(xml_file != NULL)
    {
        ...
        fprintf(xml, "<arrow_bound type=\"DCBPB\" ");
